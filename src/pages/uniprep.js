...
export async function getServerSideProps(context) {
  ...
  if (!response.ok) {
    ...
    destroyCookie(context, 'access_token');
    destroyCookie(context, 'refresh_token');
    return { redirect: { destination: '/?message=unauthorized', permanent: false } };
  }
         ...
         <div className="relative flex-1 z-30">
          <div className="customScroll relative rounded-t-xl max-h-[350px] md2:max-h-[calc(100vh-100px)] overflow-y-auto pr-2">
           <div className="flex flex-col gap-12 justify-end relative bg-white h-full min-h-[350px] md2:min-h-[calc(100vh-100px)] rounded-xl pb-3">
            ...
            <div className="right flex justify-end"><div className="bg-[#EFF7FF] text-main-dark px-4 py-3" style={{borderRadius: "20px 20px 0px 20px"}}>I need to find a job in UK. Help me steps to find a job. Break down each step and help me master each and build a award winning profile</div></div>
            <div className="left flex"><div className="bg-[#E8EAEE] text-[#595959] px-4 py-3" style={{borderRadius: "20px 20px 20px 0px"}}>Hey Tanya, sure! Lets build you impress prodfile so as to aad jfsd hgdfshsj ejjhads jd jgdas dfk dfjad </div></div>
           </div>
          </div>
         </div>
